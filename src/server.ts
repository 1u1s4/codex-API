...
export function createCodexServer(options: CreateCodexServerOptions = {}) {
  const apiKey = normalizeNonEmptyString(options.apiKey) ?? normalizeNonEmptyString(process.env.CODEX_SERVER_API_KEY);
  if (!apiKey) {
    throw new Error("Missing CODEX_SERVER_API_KEY. Set it before starting the server.");
  }

  const host = normalizeNonEmptyString(options.host) ?? normalizeNonEmptyString(process.env.CODEX_SERVER_HOST) ?? DEFAULT_SERVER_HOST;
  const port =
    ...
    try {
      ...
      if (req.method === "POST" && pathname === "/v1/responses") {
          ...
          try {
            for await (const event of stream.events) {
              ...
              if (event.type === "response.failed") {
                const message =
                  typeof (event as { response?: { error?: { message?: unknown } } }).response?.error?.message === "string"
        ...
        if (result.status >= 400) {
          const message =
            typeof (result.body as { error?: { message?: unknown } } | null)?.error?.message === "string"
          ...
          writeOpenAiError(res, status, message, status === 400 ? "invalid_request_error" : "server_error");
          return;
        }
      ...
      if (pathname === "/v1/responses" || pathname === "/v1/models" || pathname.startsWith("/v1/models/")) {
        writeOpenAiError(res, 405, `Method ${req.method ?? "UNKNOWN"} not allowed.`, "invalid_request_error");
        return;
      }
