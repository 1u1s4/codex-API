...
describe("createCodexClient CLI backend", () => {
  ...
  it("maps a fast gpt-5.4 request to Codex CLI config overrides", async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), "codex-cli-fast-"));
    const sessionFile = path.join(dir, "codex-sessions.json");
    const auth = createAuthStub(sampleCredential);
    const execFileFn = vi.fn().mockResolvedValue({
      stdout: JSON.stringify({
        thread_id: "thread-fast",
        item: { type: "assistant_message", text: "Fast reply" },
      }),
      stderr: "",
    });
    ...
    expect(result.outputText).toBe("Fast reply");
    expect(result.responseState).toMatchObject({
      backend: "cli",
  ...
  it("passes through explicit CLI reasoning overrides", async () => {
