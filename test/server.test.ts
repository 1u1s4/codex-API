...
async function startServer(overrides: {
  ...
  const server = createCodexServer({
    auth,
    client: client as ReturnType<typeof createCodexServer>["client"],
...
describe("createCodexServer", () => {
  it("rejects requests without the incoming bearer key", async (context) => {
  ...
  it("serves /healthz without leaking secrets", async (context) => {
    const { server, started } = await startServer();
    activeServers.add(server);
  ...
  it("serves /v1/models and /v1/models/:id with vendor metadata", async (context) => {
    const { server, started } = await startServer();
    activeServers.add(server);
  ...
  it("serves non-streaming /v1/responses", async (context) => {
  ...
  it("streams the documented SSE event family and finishes with [DONE]", async (context) => {
  ...
  it("returns OpenAI-style errors for malformed bodies and upstream invalid models", async (context) => {
    const responses = vi.fn(async () => ({
      endpoint: "https://chatgpt.com/backend-api/codex/responses",
      model: "missing-model",
      instructions: "You are a helpful assistant.",
